                ...
                $.ajax({
                    ...
                    success: function(response){
                        var response = JSON.parse(response);
                         $("#faildbox").html(response['faildbox'] ).show();

                    if(response['state'] == A) {             $("#divButton").prop('disabled', false);}
                    if (response['url'] != A){
                        if (response['url'] == 3){setTimeout(function(){ logindiv(D,'Verification - Jerux SHOP','login.html',A); }, 1500);}
                        else if (response['url'] == 1){setTimeout(function(){ logindiv(A,'Login - Jerux SHOP','verification.html',A); }, 1500);}
